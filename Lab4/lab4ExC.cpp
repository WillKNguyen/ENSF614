...
void print_from_binary(char* filename) {
    ...
    ofstream out_stream(strcat(filename, ".txt"));
    if(in_stream.fail())
    {
        cout <<"Error opening file..." << endl;
    }
    while (in_stream.read((char*)(&city), sizeof(City)))
    {
        cout << "Name: " << city.name << ", x coordinate: " << city.x << ", y coordinate: " << city.y << endl;
        out_stream << "Name: " << city.name << ", x coordinate: " << city.x << ", y coordinate: " << city.y << endl;
    }
    in_stream.close();
    out_stream.close();
}
